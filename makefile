...
$(OBJ_DIR)/libasm.o: $(SRC_ASM) | $(OBJ_DIR)
	@COUNT=1; PERCENT=$$((COUNT*200/$(TOTAL))); printf "[%3d%%] Assembling $<\n" $$PERCENT; $(AS) -f$(VERSION) $< -o $@


$(OBJ_DIR)/libc.o: $(SRC_LIBC) | $(OBJ_DIR)
	@COUNT=2; PERCENT=$$((COUNT*200/$(TOTAL))); printf "[%3d%%] Compiling $<\n" $$PERCENT; $(GCC) -no-pie -c $< -o $@

$(LIB): $(LIB_OBJ)
	@COUNT=3; PERCENT=$$((COUNT*200/$(TOTAL))); printf "[%3d%%] Archiving library\n" $$PERCENT; $(AR) rcs $@ $^

$(OBJ_DIR)/fs.o: $(SRC_MAIN) | $(OBJ_DIR)
	@COUNT=4; PERCENT=$$((COUNT*200/$(TOTAL))); printf "[%3d%%] Compiling $<\n" $$PERCENT; $(GCC) -no-pie -c $< -o $@

$(TARGET): $(MAIN_OBJ) $(LIB) | $(BIN_DIR)
	@COUNT=5; PERCENT=$$((COUNT*200/$(TOTAL))); printf "[%3d%%] Linking $@\n" $$PERCENT; $(GCC) $^ -o $@ -no-pie

run: $(TARGET)
	@COUNT=6; PERCENT=$$((COUNT*200/$(TOTAL))); printf "[%3d%%] Running $@\n" $$PERCENT; ./$(TARGET)
